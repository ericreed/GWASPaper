...
\textbf{If we want to report the number of PCs needed to account for 90\% of the variation I wrote the following code.  It suggests that 1020 PCs are needed.  10 PCs account for less than 1\% of the total variation.}
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{pervar} \hlkwb{<-} \hlkwd{cumsum}\hlstd{(evv}\hlopt{$}\hlstd{values)}\hlopt{/}\hlkwd{sum}\hlstd{(evv}\hlopt{$}\hlstd{values)}
\hlstd{var} \hlkwb{<-} \hlnum{0.9}
\hlstd{pcs} \hlkwb{<-} \hlkwd{which}\hlstd{(}\hlkwd{abs}\hlstd{(pervar} \hlopt{-} \hlstd{var)} \hlopt{==} \hlkwd{min}\hlstd{(}\hlkwd{abs}\hlstd{(pervar} \hlopt{-} \hlstd{var)))}
\hlstd{pcs}
\end{alltt}
\end{kframe}
\end{knitrout}
