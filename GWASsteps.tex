...
\hlstd{use} \hlkwb{<-} \hlstd{(}\hlopt{!}\hlkwd{is.na}\hlstd{(snpsum.col}\hlopt{$}\hlstd{MAF)} \hlopt{&} \hlstd{snpsum.col}\hlopt{$}\hlstd{MAF} \hlopt{>} \hlstd{minor)} \hlopt{&} \hlstd{(}\hlopt{!}\hlkwd{is.na}\hlstd{(snpsum.col}\hlopt{$}\hlstd{z.HWE)} \hlopt{&}
    \hlstd{snpsum.col}\hlopt{$}\hlstd{z.HWE}\hlopt{^}\hlnum{2} \hlopt{<} \hlkwd{qnorm}\hlstd{((hardy}\hlopt{/}\hlnum{2}\hlstd{)}\hlopt{/}\hlstd{comp)}\hlopt{^}\hlnum{2}\hlstd{)} \hlopt{&} \hlstd{snpsum.col}\hlopt{$}\hlstd{Call.rate} \hlopt{>=}
    \hlstd{call}  \hlcom{#This creates a Boolean Vector of 'TRUE' and 'FALSE'}
