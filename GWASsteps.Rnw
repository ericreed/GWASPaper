...
\textsc{Additive}\\
Additive models are used to evaluate additive structure and reveal associations that depend additively based on the allele classification. These models assume that if having a single recessive allele will increase the trait by some $\beta$, then having two recessive alleles in the homologs will increase the trait, on the average, by $2\beta$. This model can be written as:
...
\text{fldl_wk0}_i &=& \beta_0 \text{age}_i + \beta_1 \text{sex}_i + \beta_2 \text{race}_i + \beta_3 \text{PC}_{1,i} + \beta_4 \text{PC}_{2,i} + \cdots + \beta_{13} \text{PC}_{10,i} 
\end{eqnarray}
